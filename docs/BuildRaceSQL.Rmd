...
race50 %>%
  select(-COUNTYA) %>%
  select(year, county, TRACTID, countyID, white50, black50, other50, GISJOIN) %>%
  mutate(TRACTID = str_replace(string = TRACTID, pattern = "NA$", replacement = "")) %>%
  mutate(countyID = str_replace(string = countyID, pattern = "NA$", replacement = "")) %>%
...
stlrace50 %>%
  select(year, county, tractID, countyID, white50, black50, other50, geometry) -> stlrace50

# interpolate to 2010 census tract
stl_tract10 %>%
  st_transform(crs = 26915) %>%
  select(GEOID10) %>%
  aw_interpolate(tid = GEOID10, source = stlrace50, sid = countyID,
                 weight = "total", output = "tibble", 
                 extensive = c("white50", "black50", "other50")) -> tract50

# clean up data frame
tract50 %>%
  select(GEOID10, white50, black50, other50) %>%
  rename(GEOID = GEOID10,
         white = white50,
         black = black50,
         other = other50) %>%
...
race60 %>%
  select(-COUNTYA) %>%
  select(year, county, TRACTID, countyID, white60, black60, other60, GISJOIN) %>%
  mutate(TRACTID = str_replace(string = TRACTID, pattern = "NA$", replacement = "")) %>%
  mutate(countyID = str_replace(string = countyID, pattern = "NA$", replacement = "")) %>%
...
stlrace60 %>%
  select(year, county, tractID, countyID, white60, black60, other60, geometry) -> stlrace60

# interpolate to 2010 census tract
stl_tract10 %>%
  st_transform(crs = 26915) %>%
  select(GEOID10) %>%
  aw_interpolate(tid = GEOID10, source = stlrace60, sid = countyID,
                 weight = "total", output = "tibble", 
                 extensive = c("white60", "black60", "other60")) -> tract60

# clean up data frame
tract60 %>%
  select(GEOID10, white60, black60, other60) %>%
  rename(GEOID = GEOID10,
         white = white60,
         black = black60,
         other = other60) %>%
...
race70 %>%
  ...
  rename(countyID = county,
         ...
         white70 = C0X001,
         black70 = C0X002,
         other70 = C0X003) -> race70 

# clean
race70 %>%
  select(-COUNTYA) %>%
  select(year, county, TRACTA, countyID, white70, black70, other70, GISJOIN) %>%
...
stlrace70 %>%
  select(year, county, tractID, countyID, white70, black70, other70, geometry) -> stlrace70

# interpolate to 2010 census tract
stl_tract10 %>%
  st_transform(crs = 26915) %>%
  select(GEOID10) %>%
  aw_interpolate(tid = GEOID10, source = stlrace70, sid = countyID,
                 weight = "total", output = "tibble", 
                 extensive = c("white70", 
                               "black70", 
                               "other70")) -> tract70
...
# clean up data frame
tract70 %>%
  select(GEOID10, white70, black70, other70) %>%
  rename(GEOID = GEOID10,
         white = white70,
         black = black70,
         other = other70) %>%
...
race80 %>%
  ...
  mutate(other80 = C9D003 +
                   ...
                   C9D014 +
                   C9D015) %>%
  select(-C9D003, -C9D004, -C9D005, 
         -C9D006, -C9D007, -C9D008, 
         -C9D009, -C9D010, -C9D011, 
         -C9D012, -C9D013, -C9D014, -C9D015) %>% 
  rename(countyID = county,
         year = YEAR,
         county = COUNTY,
         white80 = C9D001,
...
race80 %>%
  select(-COUNTYA) %>%
  select(year, county, TRACTA, countyID, white80, black80, other80, GISJOIN) %>%
...
stlrace80 %>%
  select(year, county, tractID, countyID, white80, black80, other80, geometry) -> stlrace80

# interpolate to 2010 census tract
stl_tract10 %>%
  st_transform(crs = 26915) %>%
  select(GEOID10) %>%
  aw_interpolate(tid = GEOID10, source = stlrace80, sid = countyID,
                 weight = "total", output = "tibble", 
                 extensive = c("white80", 
                               "black80", 
                               "other80")) -> tract80
...
# clean up data frame
tract80 %>%
  select(GEOID10, white80, black80, other80) %>%
  rename(GEOID = GEOID10,
         white = white80,
         black = black80,
         other = other80) %>%
...
race90 %>%
  ...
  select(-STATE, -STATEA, -CTY_SUBA,
         ...
         -CD103A, -ANPSADPI) %>%
  mutate(county = paste0(COUNTYA, "-", TRACTA)) %>%
  mutate(other90 = EUY003 +
                   EUY004 +
                   EUY005) %>%
  select(-EUY003, -EUY004, -EUY005) %>% 
  rename(countyID = county,
         year = YEAR,
         county = COUNTY,
         white90 = EUY001,
...
race90 %>%
  select(-COUNTYA) %>%
  select(year, county, TRACTA, countyID, white90, black90, other90, GISJOIN) %>%
...
rbind(stlcity90, stlcounty90) -> stlrace90
stlrace90 <- left_join(stlpop90, pop90, by = "pop90")

# join data to census tract shapefile
stlrace90 <- left_join(stl80, race80, by = "GISJOIN")
stlrace90 %>%
  select(year, county, tractID, countyID, white80, black80, other80, geometry) -> stlrace90
...
# clean up environment
rm(tract80, stl80, stlrace80, stlcity90, stlcounty90)
```
